# as in 1b but with extra non-linearity for error neurons
...
class NeuronGroup:
    ...
    def __init__(self, name, num_circuits, num_error_pairs=2, pos_error_to_head=(1, 0), neg_error_to_head=(0.5, 0),
        ...
        self.head_in_error = np.zeros(num_circuits)
        self.head_external = np.zeros(num_circuits)

        # input to negative error neurons
        self.current_input = np.zeros((num_error_pairs, num_circuits))
    ...
    def step(self, external_input=None):
        ...
        # calculate inputs to neg_error neurons
        self.head_in_error += (-self.head_in_error + self.head_out) / self.fast_time_constant
        ...
        for error_pair in self.target_error_pairs:
            ...
            self.current_input[error_pair] = self.dendrite_nonlinearity(np.dot(input_values, self.weights_from[error_pair].weights[error_pair]))

        if external_input is not None:
            self.current_input += external_input
        self.neg_error_input += (- self.neg_error_input + self.current_input) / self.fast_time_constant

        # update error neurons
        self.neg_error_diff = -self.neg_error
        self.neg_error = self.neg_error_input - self.head_in_error
        self.neg_error_diff += self.neg_error
        ...
        self.neg_error_out = np.maximum(0, 1 / (1 + np.exp(-50*(self.neg_error_input-0.75))) - self.head_in_error)
        if self.log_neg_error_out:
            self.neg_error_out_log.append(self.neg_error_out)

        self.pos_error_out = np.maximum(0,  self.head_in_error - 1 / (1 + np.exp(-50*(self.neg_error_input-0.25))))
